...
| **Hero** | Full-width gradient + wave, Image bg overlay, Solid orange |
...
- Web typography scale (px/rem equivalents to PPTX dimensions)
- Spacing system
- SVG waves (reused from brand-guidelines.md, adapted for HTML)
...
**Critical accessibility note**: `#FF5800` on `#FFFFFF` has contrast **2.8:1** (WCAG AA fail for normal text). Rule: orange only for text ≥24px bold (large text — ratio OK ≥3:1) or as decorative/CTA element. Reading text always in `#333333`.
...
- **Step 3: Generate** — ready-to-use code, commented per section
...
- `bundles.json`: add `ava-web` to `content` bundle (branded content) and to `all`
...
- [ ] No date in SKILL.md (dates only in README.md Metadata table)
