...
2. Let skill-creator conduct the interview, write the SKILL.md, and create the evals
...
| `gtm-product-` | Exclusive to SaaS/PLG Product GTM | `gtm-product-launch` |
...
- `GTM-Strategist/gtm-strategist-skills` — Maja Voje methodology, 12 phases, tested in 1000+ companies. Skills in `.claude/skills/` in the `master` branch.
...
> Invoke `gtm-consulting-workflow` or `gtm-product-workflow` to execute the full workflow.
...
Support (any step):
  ...
  mckinsey-strategist       → Complex problem structure
```

---

### Workflow B — SaaS/PLG Product GTM
...
```
  gtm-market-sizing   →  Used in BOTH workflows
  gtm-icp-designer    →  Used in BOTH workflows
...
**Original diagnosis:** Stub disguised as a skill. Phase 5 empty. No ICP. No templates. Written as a role description ("You are..."). Covered 5 phases belonging to 3 distinct skills.

**Design decision:**
- `market-sizing` [NEW] — TAM/SAM/SOM reusable across both workflows
...
**Arquivo:** `skills/startup-growth-strategist/SKILL.md`
**Action:** REPLACE the entire current content with the content below.
...
The skill is solid, but 7 specific additions are missing. All are surgical — do not rewrite the entire skill.
...
**Action:** Insert after that table:
...
**Arquivo:** `skills/product-strategy/SKILL.md`
**Location:** In the "Quick Reference: Strategy Quality Checklist" section, after the existing 9 items
...
The skill is strong — 3 surgical high-precision additions.
...
**Purpose:** The missing 4th P in both workflows. Positioning is the multiplier — with it right, ICP converts faster, the channel performs better, and price justifies itself.
...
Purpose: Define market positioning and messaging using April Dunford's 6-step framework,
create a Messaging House with per-persona variations, develop battle cards per
...
Evals (3 test cases):
1. "Position our AI contract review tool — competitors are DocuSign, Ironclad, and manual review"
2. "Our message is 'enterprise analytics platform' and it is not converting — where is it wrong?"
3. "Create battle card for our Outsourcing proposal vs. the client's internal team" (consulting context)
```
...
**Purpose:** Most important skill in Track B. Guides the complete journey from GTM canvas to go-live in 9 sequential phases. Based on Maja Voje (GTM-Strategist) methodology + gtm-skills-os. Each phase produces an output that feeds the next.
...
**Purpose:** WTP research, value metric selection, tier design, competitive anchoring. Based on the gtm-skills-os pricing-strategy — the most rigorous of the 3 repos on this topic.
...
> **Reference:** The complete content of each skill is defined in `docs/plan/2026-04-03-gtm-consulting-skills-plan-v2.md`. This plan supersedes v2 — execute from the content here and ignore v2 for the batch sequence. The skill content (SKILL.md body) is fully available in v2.
...
methodology, team structure, Hero Offers as entry points, and a
...
- "Help me design the deliverables for [offer]"
...
Evals:
1. "Structure a Hero Offer for an AI Assessment for a Whitespace account in the Manufacturing sector"
2. "Design the deliverables for a $500K Azure SI engagement in the financial sector"
...
rate premium, and structure pricing by offer type and account tier.
...
- "How do I qualify this deal?" → gtm-consulting-qualifier
...
Evals:
1. "Is T&M or fixed-fee better for an $800K SI in healthcare — the client is risk-conservative"
2. "Justify our $220/hour rate for an Azure Architect when offshore costs $80"
...
**Execution instruction:** Continue reading v2 for the content of each skill. If any skill does not have complete content in v2, create it based on the standards established in this plan.
...
**Prerequisite:** All Tracks A, B, C completed and approved by user.
...
### Added (16 new skills with gtm- / gtm-product- / gtm-consulting- naming convention)
